...
def beta2(sym):
    pre=[h for h in range(4,9) if h in HV[sym] and h in HV['ES_F']]
    return st.mean([HV[sym][h]/HV['ES_F'][h] for h in pre]) if pre else 1.0
def tiervar(sym, weekend_ref=False):
    ...
    for wd in range(7):
        for h in range(24):
            ...
            if key=='CLOSED':
                T[key].append(HV['ES_F'].get(20,0)*b if (wd==6 and h>=18) else 0.0)
            else:
                T[key].append(HV[sym].get(h, HV['ES_F'].get(h,0)*b))
...
json.dump(out, open('tiervar_all.json','w'))
