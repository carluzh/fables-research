import { poolStatsOf, aprPctOf } from '/Users/carlschmidt/Desktop/Projects/fables-ui/src/data/gw-pool'
...
type Row = { key: string; pair: string; side: 'fables' | 'rival'; id: string; v: 3 | 4; note: string }
const ROWS: Row[] = [
  // Fables (v4, hooked, dynamic)
...
async function batch(rows: Row[]) {
  const args = rows.map((_, i) => `$p${i}: String!`).join(', ')
  const roots = rows
    .map((r, i) => (r.v === 4 ? `  a${i}: v4Pool(chain: $chain, poolId: $p${i}) ${F4}` : `  a${i}: v3Pool(chain: $chain, address: $p${i}) ${F3}`))
    .join('\n')
  const query = `query B($chain: Chain!, ${args}) {\n${roots}\n}`
  const variables: Record<string, unknown> = { chain: 'ROBINHOOD' }
  rows.forEach((r, i) => (variables[`p${i}`] = r.id))
  const res = await fetch(GQL, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', origin: ORIGIN }, body: JSON.stringify({ operationName: 'B', query, variables }) })
  const j: any = await res.json()
  if (j.errors) console.error('GQL ERRORS', JSON.stringify(j.errors).slice(0, 800))
  return j.data ?? {}
}

async function hist(id: string, v: 3 | 4, dur: string) {
  const res = await fetch(HIST, { method: 'POST', headers: { 'content-type': 'application/json', accept: 'application/json', origin: ORIGIN }, body: JSON.stringify({ pool: { chainId: 'ROBINHOOD', addressOrId: id, version: v === 4 ? 3 : 2 }, duration: 'HISTORY_DURATION_' + dur }) })
  const j: any = await res.json()
  return (j.buckets ?? []).map((x: any) => ({ t: Number(x.timestamp), v: Number(x.volumeUsd) || 0, f: Number(x.feeUsd) || 0 }))
}
...
for (let i = 0; i < ROWS.length; i++) {
  const r = ROWS[i]!
  const node = data[`a${i}`]
  if (!node) { out.push({ ...r, err: 'no node' }); continue }
  const st = poolStatsOf(node)
  const b = await hist(r.id, r.v, 'WEEK')
  out.push({ ...r, tvlUsd: st.tvlUsd, tvlChangePct: st.tvlChangePct, feeTier: st.feeTier, isDynamicFee: st.isDynamicFee, hook: node.hook?.address ?? null, txCount: Number(node.txCount) || null, buckets: b })
  await new Promise((s) => setTimeout(s, 120))
}
fs.writeFileSync('/private/tmp/claude-501/-Users-carlschmidt-Desktop-Projects/fae2a16e-daa5-42c7-bfe1-211269ebbec0/scratchpad/now.json', JSON.stringify({ fetchedAt: new Date().toISOString(), out }))
console.log('rows', out.length, 'at', new Date().toISOString())
for (const o of out) {
  const n = (o.buckets ?? []).length
  console.log(`${o.key.padEnd(16)} tvl=${o.tvlUsd == null ? 'null' : Math.round(o.tvlUsd)} buckets=${n} last=${n ? new Date(o.buckets[n - 1].t * 1000).toISOString() : '-'}`)
}
